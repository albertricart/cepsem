...
class Utilitat
{
    ...
    public static function checkIncidenciaHasRecurs($incidencia_has_recurs){
        if (empty($incidencia_has_recurs->incidencies_id)) {
            $response = \response(['errorMessage' => "El camp incidencies_id  és obligatori"]);

        } else if (empty($incidencia_has_recurs->recursos_id)) {
            $response = \response(['errorMessage' => "El camp recursos_id  és obligatori"]);

        } else if (empty($incidencia_has_recurs->afectats_id)) {
            $response = \response(['errorMessage' => "El camp afectats_id  és obligatori"]);
    ...
    public static function getIncidenciaHasRecurs($incidencia_has_recurs)
    {
        ...
        $incidenciaHasRecurs->afectats_id = $incidencia_has_recurs->afectats_id;
    ...
    public static function checkIncidencia($request)
    {
        if (empty($request->input('num_incident'))) {
            $response = \response(['errorMessage' => "El camp num_incident és obligatori"]);
        } else if (empty($request->input('data'))) {
            $response = \response(['errorMessage' => "El camp data és obligatori"]);
        } else if (empty($request->input('hora'))) {
            $response = \response(['errorMessage' => "El camp hora és obligatori"]);
        } else if (empty($request->input('telefon_alertant'))) {
            $response = \response(['errorMessage' => "El camp telefon_alertant és obligatori"]);
        } else if (empty($request->input('adreca'))) {
            $response = \response(['errorMessage' => "El camp adreça és obligatori"]);
        } else if (empty($request->input('descripcio'))) {
            $response = \response(['errorMessage' => "El camp descripció és obligatori"]);
        } else if (empty($request->input('tipus_incidencies_id '))) {
            $response = \response(['errorMessage' => "El camp tipus_incidencies_id  és obligatori"]);
        } else if (empty($request->input('alertants_id '))) {
            $response = \response(['errorMessage' => "El camp alertants_id  és obligatori"]);
        } else if (empty($request->input('municipis_id '))) {
            $response = \response(['errorMessage' => "El camp municipis_id  és obligatori"]);
        } else if (empty($request->input('usuaris_id '))) {
            $response = \response(['errorMessage' => "El camp usuaris_id  és obligatori"]);
        } else {
            $response = '';
        }
