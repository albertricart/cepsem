...
@section('content')
    @if ($idincidencia == -1)
        <incidenciaform-component :insert="true" :tipusalertants="{{ $tipusAlertants }}" :provincies="{{ $provincies }}" :tipusincidents="{{ $tipusIncidents }}" :sexes="{{ $sexes }}" :tipusrecursos="{{ $tipusRecursos }}"></incidenciaform-component>
    @else
